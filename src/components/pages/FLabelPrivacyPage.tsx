...
export default function FLabelPrivacyPage() {
  return (
    <div className={styles.page}>
      <PageHero
        label="PRIVACY"
        title="Fラベル プライバシーポリシー"
        description="本アプリにおける個人情報の取り扱いについて定めるものです。"
      />
          ...
          <article className={`glass-panel ${styles.card} reveal`}>
            ...
            <section className={styles.section}>
              ...
              <p className={styles.paragraph}>
                本アプリは、マーチャントがインストール時に付与したアクセススコープの範囲内で、Shopify
                Admin APIを通じて以下の情報にアクセスします。
              ...
              <ul className={styles.list}>
                ...
                <li>
                  商品メタフィールド（食品表示データ:
                  原材料、アレルゲン、栄養成分、原料原産地、内容量、賞味期限、事業者情報等）
                </li>
                <li>
              ...
              <p className={styles.paragraph}>
                本アプリが要求するアクセススコープ: read_products,
                write_products, read_metaobject_definitions,
                write_metaobject_definitions, read_metaobjects,
                write_metaobjects
              </p>
              ...
              <ul className={styles.list}>
                <li>
                  セッション情報:
                  Shopifyショップドメイン、OAuthアクセストークン、スコープ情報、有効期限
                </li>
                <li>
                  アプリ設定情報:
                  ショップごとの初期セットアップ完了日時等、アプリ動作に必要な内部状態
                </li>
              </ul>
              <p className={styles.paragraph}>
              ...
              <p className={styles.paragraph}>
                <strong>
                  本アプリは、マーチャントの顧客から、いかなる個人情報も直接取得しません。
                </strong>
                ストアフロントに表示されるTheme App
                Extension（食品表示ブロック）は、マーチャントが登録した食品表示データを表示するのみで、
              ...
              <ul className={styles.list}>
                ...
                <li>
                  管理画面における認証は、Shopify App
                  Bridgeが発行するセッショントークン（JWT）を使用し、サードパーティCookieに依存しません。
                </li>
                <li>
                  本アプリは、Google
                  Analytics等の外部アクセス解析ツール、広告トラッキングピクセル、ブラウザフィンガープリンティング技術を一切使用しません。
                </li>
              </ul>
            </section>
                ...
                <table className={styles.table}>
                  ...
                  <tbody>
                    <tr>
                      <td>Shopify Inc.</td>
                      <td>
                        Shopifyプラットフォーム、Admin API、Billing
                        API、OAuth認証、Webhook配信
                      </td>
                      <td>カナダ／米国</td>
                    </tr>
                    <tr>
                      <td>クラウドホスティング事業者</td>
                      <td>
                        アプリケーションホスティング、PostgreSQLデータベース運用
                      </td>
                      <td>海外（複数リージョン）</td>
                    </tr>
                  </tbody>
            ...
            <section className={styles.section}>
              ...
              <ul className={styles.list}>
                <li>
                  セッション情報・アプリ設定情報:
                  マーチャントが本アプリをインストールしている期間中、またはShopifyがトークンを失効させるまで保存します。
                </li>
                <li>
                  アプリアンインストール時: Shopifyから送信される
                  app/uninstalled
                  Webhookを受信した時点で、セッション情報を速やかに削除します。
                </li>
                <li>
                  ショップデータ削除請求時: Shopifyから送信される shop/redact
                  Webhook（アンインストールから48時間経過後に送信）を受信した時点で、当該ショップに紐づくすべての当方保存データを完全に削除します。
                </li>
                <li>
                  ログデータ:
                  運営上の必要性に応じた期間（原則30日間）保持後、順次削除されます。
                </li>
                <li>
                  バックアップ:
                  システムバックアップに含まれるデータは、削除後最大30日間システム上に残存する可能性がありますが、新規処理には一切利用されません。
                </li>
              </ul>
            </section>
            ...
            <section className={styles.section}>
              ...
              <ul className={styles.list}>
                <li>
                  通信の暗号化: クライアント・サーバー間のすべての通信をTLS
                  1.2以上で暗号化
                </li>
                <li>
                  アクセストークンの保護:
                  OAuthアクセストークンはデータベースに暗号化したうえで保存
                </li>
                <li>
                  Webhook真正性検証:
                  ShopifyからのWebhook送信はHMAC署名により都度検証
                </li>
                <li>
                  アクセス制御:
                  本番データベースへのアクセスは権限管理された環境下でのみ実施
                </li>
                <li>
                  不要データの自動削除:
                  アプリアンインストール時のセッション自動削除、データ削除Webhook対応
                </li>
              </ul>
            </section>
            ...
            <section className={styles.section}>
              ...
              <ul className={styles.list}>
                ...
                <li>
                  データの削除:
                  Shopify管理画面から本アプリをアンインストールすることで、自動的にデータ削除プロセスが開始されます
                </li>
                <li>
                  利用停止: Shopify App
                  Store記載のサポートメールアドレスへの連絡
                </li>
                <li>
                  データポータビリティ（GDPR適用時）:
                  構造化された形式でのデータ提供請求
              ...
              <ul className={styles.list}>
                <li>
                  ...
                  当方が当該顧客に関連するデータを保有していない旨を応答します
                </li>
                <li>
                  customers/redact:
                  削除対象となるデータが存在しないことを確認し、処理ログを記録します
                </li>
                <li>
                  shop/redact:
                  アンインストールから48時間以内に受信し、当該ショップに紐づくすべての保存データを削除します
                </li>
              </ul>
            </section>
